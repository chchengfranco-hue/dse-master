...
// Renders one item as HTML string for html2canvas capture
function buildItemHtml(item, mod, idx) {
  const title = mod.getTitle(item);
  const meta = mod.getMeta(item);
  const body = (mod.getBody(item) || '').replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>').replace(/_(.*?)_/g, '<em>$1</em>').replace(/\n/g, '<br/>');
  ...
  const annotHtml = annotEntries.length > 0 ? `
    <div style="margin-top:18px;background:#fefce8;border:1px solid #fde68a;border-radius:8px;padding:12px 14px;">
      <div style="font-size:11px;font-weight:700;color:#92400e;margin-bottom:8px;letter-spacing:0.05em;">VOCABULARY &amp; ANNOTATIONS</div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px 16px;">
        ${annotEntries.map(([word, meaning]) => `
          <div style="display:flex;gap:6px;align-items:baseline;">
            <span style="font-weight:700;color:#5b21b6;white-space:nowrap;font-size:11px;">${word}</span>
            <span style="color:#444;font-size:10.5px;">${meaning}</span>
          </div>
        `).join('')}
      </div>
    </div>
  ...
  return `
    <div style="font-family:system-ui,-apple-system,'Noto Sans TC','Microsoft JhengHei','PingFang TC',sans-serif;padding:24px 28px;background:#fff;width:740px;box-sizing:border-box;">
      <div style="border-left:4px solid #7c3aed;padding-left:12px;margin-bottom:14px;">
        <div style="font-size:9px;color:#7c3aed;font-weight:600;letter-spacing:0.08em;margin-bottom:4px;">${String(idx + 1).padStart(2, '0')}</div>
        <div style="font-size:16px;font-weight:700;color:#1a1a1a;line-height:1.4;">${title}</div>
        ${meta ? `<div style="font-size:10px;color:#7c3aed;margin-top:3px;">${meta}</div>` : ''}
      </div>
      <div style="font-size:12px;color:#333;line-height:1.9;white-space:pre-wrap;">${body}</div>
      ${annotHtml}
    </div>
  `;
}
...
export default function GlobalPdfExport({ onClose }) {
  ...
  const exportPdf = async () => {
    ...
    // ── Cover page (pure shapes + text, no CJK on cover) ──
    doc.setFillColor(88, 28, 180);
    doc.rect(0, 0, PW, PH, 'F');
    doc.setFillColor(109, 40, 217);
    doc.rect(0, PH - 55, PW, 55, 'F');
    doc.setFontSize(10); doc.setFont('helvetica', 'normal'); doc.setTextColor(196, 181, 253);
    doc.text('HKDSE LEARNING HUB', 15, 38);
    doc.setFontSize(28); doc.setFont('helvetica', 'bold'); doc.setTextColor(255, 255, 255);
    doc.text(mod.label, 15, 58);
    doc.setDrawColor(167, 139, 250); doc.setLineWidth(0.5);
    doc.line(15, 68, 55, 68);
    doc.setFontSize(10); doc.setFont('helvetica', 'normal'); doc.setTextColor(196, 181, 253);
    doc.text(`${selectedItems.length} item${selectedItems.length !== 1 ? 's' : ''}`, 15, 78);
    doc.text(`Generated: ${new Date().toLocaleDateString('en-HK', { year: 'numeric', month: 'long', day: 'numeric' })}`, 15, 86);
    ...
    for (let idx = 0; idx < selectedItems.length; idx++) {
      ...
      // Paginate: if content is taller than one page, split across pages
      let remainH = imgH;
      let srcY = 0;
      ...
      while (remainH > 0) {
        const pageImgH = Math.min(remainH, PH);
        ...
        doc.addImage(pageImgData, 'JPEG', 0, 0, imgW, pageImgH);
